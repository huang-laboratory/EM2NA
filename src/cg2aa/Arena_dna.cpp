...
"Construction of Full-Atom structure from P-C4'-N1/N9 backbone (DNA version)\n"
"Usage: cg2aa_dna input.pdb output.pdb [option]\n"
...
int main(int argc,char **argv)
{
    ...
    // MOD by TL Postprocess residue name
    for(int i = 0;i < pdb_entry.chains.size(); ++i){
        for(int k = 0; k < pdb_entry.chains[i].residues.size(); ++k){
            if (pdb_entry.chains[i].residues[k].resn == "  U"){
                pdb_entry.chains[i].residues[k].resn =  "  T";
            }
        }
    }
