...
bool msshift(MRC *m, Points *p, float resol, float max_shift){
    time_t tick0 = time(0);
...
bool msmerge(MRC *m, Points *p, double cut){
    time_t tick0 = time(0);
    double mpix = 1.0; //default = 0.5
    ...
    for(int i=0;i<p->Ncd-1;i++){
        double tmp[3], d2;
        if((p->dens[i]-dmin)*rv_range < rdcut)//discard points with normalized density ratio below rdcut
            stock[i]=false;
